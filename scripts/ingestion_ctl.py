...
_MERCHANT_CATS = ["crypto_exchange", "electronics", "grocery", "travel",
                  "clothing", "restaurants", "gaming", "healthcare"]
...
def status():
    ...
    try:
        ...
        raw_files = [
            obj for page in paginator.paginate(Bucket=cfg.datalake_bucket, Prefix="raw/")
            for obj in page.get("Contents", [])
            if obj["Key"].endswith(".parquet")
        ]
        quar_files = [
            obj for page in paginator.paginate(Bucket=cfg.datalake_bucket, Prefix="quarantine/")
            for obj in page.get("Contents", [])
            if obj["Key"].endswith(".parquet")
        ]
